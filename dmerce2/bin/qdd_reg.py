...
class NewScratch:
    ...
    def ActivateEmail(self, fqhn, email, id):
        ...
        s = "INSERT INTO dmerce_sys.SendEmail (ID, FQHN, Template, Query, ToDo, PrjDocRoot)" \
            " VALUES ('%s', 'www.register.1ci.de', 'Email,email_scratch_okay', '%s', 1, " \
            " '/export/honey/www/1ci.de/register/www')" % (str(dboid), query)
        self.__sql[s]
    ...
    def UpdateDatabase(self):
        t = time.time()
        s = "UPDATE dmerce_sys.ManageProjects SET ToDo = 0, ChangedDateTime = %f, VALIDFROM = %f,"\
            " VALIDUNTIL = %f, EMAILSENDDATE = %f, PROJECTIP = '%s', PROJECTNAME = '%s'," \
            " PROJECTDBNAME = '%s' WHERE ID = '%s'" % (t, t, self.__tPlus15Days, t, self.__projectIpAddr,
                                                       self.__projectName, self.__projectDbName,
                                                       str(self.__rqID))
        self.__sql[s]
        s = "INSERT INTO %s.Agent (ID, CreatedDateTime, active, Login, passwd)" \
            " VALUES (1, %f, 1, 'demo', '%s')" % (self.__projectDbName, t,
                                                  self.__projectDbName + str(os.getpid()))
        self.__sql[s]
        dnsRecordId = self.__nccDboid['DnsRecords']
        s = "INSERT INTO dmerce_ncc.DnsRecords (ID, ZoneID, active, Name, Type, Value)" \
            " VALUES (%i, 4, 1, '%s', 6, '%s')" % (dnsRecordId, self.__recordName, self.__projectIpAddr)
        self.__sql[s]
        vhostID = self.__nccDboid['SrvWebsrvVhosts']
        s = "INSERT INTO dmerce_ncc.SrvWebsrvVhosts (ID, active, ToDo, ServerID, SrvWebsrvConfigID," \
            " AccessLogType, DmerceVersion, ServerAdmin, DnsZoneID, DnsRecordID)" \
            " VALUES (%s, 1, 1, 7, 1, 2, 4, 'webmaster@customer.1ci.de', 4, %i)" % (vhostID, dnsRecordId)
        self.__sql[s]
        s = "UPDATE dmerce_ncc.DnsZones SET ToDo = 1 WHERE Name = '1ci.de'"
        self.__sql[s]

    def SetToDo(self, id):
        s = "UPDATE dmerce_sys.ManageProjects SET ToDo = 0 WHERE ID = %i" % id
        self.__sql[s]

    def Scratch(self):
        return os.system('%s/bin/dmerce crprj quiet=1 scratch=%s db=%s mysqlrootpwd=%s hostserial=%s projectfqhn=%s projectdestdir=%s projectdbname=%s projectuploaddir=/export/honey/www/%s/pic/upload' % (self.__conf['dmerce_home'], self.__scratch, self.__projectDbType, self.__dbPwd, self.__projectHostSerial, self.__projectFQHN, self.__projectDestDir, self.__projectDbName, self.__projectDestDir))
...
def main():
    lock = DMS.Lock.Lock('dmercereg.lock')
    if not lock.CheckLock():
        print 'dmercereg script such running'
        sys.exit()
    lock.AquireLock()
    conf = check()
    # connect to database
    c = DMS.SQL.Layer1('MySQL:dmerce_sys:dmerce_sys@localhost:dmerce_sys')
    c.Init()
    c.Connect()
    q = c.GetQuery()
    ...
    if rc:
        ...
        for i in range(rc):
            ...
            s = NewScratch(conf)
            ippool = DMS.IP.Pool(q)
            ipaddr = ippool.Get(conf['ipnet'])
            if ipaddr:
                scratch = r[i]['Scratch']
                s.SetSQL(q)
                s.SetNCCSQL(nccQ)
                ...
                s.SetDbPwd(conf['pwd'])
                s.SetProjectDbName('dmerce_' + s.GetProjectName())
                ...
                if string.replace(str(r[i]['ProjectType']) ,'L', '') == '1':
                    print t(), "  IP-ADDRESS FROM IP-POOL: '" + ipaddr + "'"
